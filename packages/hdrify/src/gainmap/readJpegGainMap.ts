...
import {
  extractImagesBySecondSoi,
  extractImagesFromMpf,
} from './readJpegGainMap/mpfExtractor.js';
...
export function readJpegGainMap(buffer: Uint8Array): FloatImageData {
  ...
  if (sdrDecoded.width !== gainDecoded.width || sdrDecoded.height !== gainDecoded.height) {
    gainData = new Uint8ClampedArray(
      scaleToSize(
        gainDecoded.data as Uint8Array,
        gainDecoded.width,
        gainDecoded.height,
        width,
        height,
...
function scaleToSize(
  ...
  for (let y = 0; y < dh; y++) {
    ...
    for (let x = 0; x < dw; x++) {
      ...
      out[dstIdx] = src[srcIdx] ?? 0;
      out[dstIdx + 1] = src[srcIdx + 1] ?? 0;
      out[dstIdx + 2] = src[srcIdx + 2] ?? 0;
      out[dstIdx + 3] = src[srcIdx + 3] ?? 255;
    }
  }
  return out;
}
