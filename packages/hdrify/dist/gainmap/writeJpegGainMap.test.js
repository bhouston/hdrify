...
function parseMpfImage2Offset(mpfPayload) {
    if (mpfPayload.length < 4 + 8 + 2)
        return null;
    const le = mpfPayload[4] === 0x49 && mpfPayload[5] === 0x49;
    const b = (o) => mpfPayload[o] ?? 0;
    const getU32 = (o) => le
    ...
    if (mpfPayload.length < mpEntriesStart + 32)
        return null;
...
function findApp2Segments(jpeg) {
    ...
    if (mpfOffset === -1 || !mpfPayload || iccOffset === -1 || !iccPayload)
        return null;
...
describe('writeJpegGainMap', () => {
    ...
    it('should produce valid adobe-gainmap format and round-trip', async () => {
        ...
        expect(result.match, `Adobe round-trip: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
    describe('ultrahdr structure (Apple Preview compatibility)', () => {
        ...
        it('should place MPF segment before ICC segment in primary image', () => {
            ...
            if (!segments)
                return;
            expect(segments.mpfOffset, 'MPF APP2 must appear before ICC APP2 for Apple Preview').toBeLessThan(segments.iccOffset);
        });
        ...
        it('should set primary image MP entry type to Baseline MP Primary Image (0x030000)', async () => {
            ...
            if (parsed.mpfImages && parsed.mpfImages.length >= 1) {
                ...
                if (!primary)
                    return;
                expect(primary.ImageType, 'Primary image type must be Baseline MP Primary Image (0x030000) for Apple Preview').toBe(BASELINE_MP_PRIMARY);
                return;
            }
            ...
            const hasSequence = Array.from({ length: payload.length - 3 }).some((_, i) => payload[i] === 0x00 && payload[i + 1] === 0x00 && payload[i + 2] === 0x03 && payload[i + 3] === 0x00);
            expect(hasSequence, 'MPF payload must contain Baseline MP Primary Image type (0x00030000) for Apple Preview').toBe(true);
        });
        it('should have MPImage2 offset pointing to JPEG SOI (valid gain map linkage)', () => {
            ...
            expect(jpegR[absOffset], `MPImage2 offset must point to SOI (0xFF); at ${absOffset} got 0x${jpegR[absOffset]?.toString(16) ?? '?'}`).toBe(MARKER_PREFIX);
            expect(jpegR[absOffset + 1], `MPImage2 offset+1 must be 0xD8; at ${absOffset + 1} got 0x${jpegR[absOffset + 1]?.toString(16) ?? '?'}`).toBe(SOI);
        });
...
//# sourceMappingURL=writeJpegGainMap.test.js.map