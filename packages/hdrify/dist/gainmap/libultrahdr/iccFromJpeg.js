...
function hasIccPrefix(payload) {
    for (let k = 0; k < ICC_PREFIX_LEN; k++) {
        if (payload[k] !== ICC_PREFIX[k])
            return false;
    }
    return true;
}
 ...
 */
export function extractIccProfileFromJpeg(jpeg) {
    let i = 0;
 ...
 */
export function getAdgcGuidFromIccProfile(profile) {
    const tableStart = ICC_HEADER_SIZE + 4;
    if (profile.length < tableStart + TAG_ENTRY_SIZE)
        return null;
    ...
    for (let i = 0; i < tagCount; i++) {
        ...
        if (offset + ADGC_GUID_OFFSET_IN_TAG + ADGC_GUID_SIZE > profile.length ||
 ...
 */
export function iccProfileHasAdgcTag(profile) {
    return getAdgcGuidFromIccProfile(profile) !== null;
}
//# sourceMappingURL=iccFromJpeg.js.map