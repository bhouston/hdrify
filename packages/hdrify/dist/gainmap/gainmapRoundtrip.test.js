...
function worstColumnsByError(original, decoded, width, height, min, max, topN) {
...
function validateSlowGradientLineByLine(original, decoded, width, height, min, max, toleranceRelative, toleranceAbsolute) {
    const refThreshold = 0.01;
    const failures = [];
    let maxError = 0;
    let firstFailureX = null;
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const p = (y * width + x) * 4;
...
function slowGradientFailureMessage(result) {
    ...
    const intensityRange = firstX !== undefined && lastX !== undefined ? [byX.get(firstX)?.intensity, byX.get(lastX)?.intensity] : null;
    const sample = sortedXs
        ...
        .join('; ');
    return (`Slow gradient: ${failures.length} pixels failed (maxError=${maxError}). ` +
...
describe('gain map in-memory round-trip (encode → decode, no JPEG)', () => {
    ...
    it('float-only encode → decode matches original within 0.5% (no quantization)', () => {
        ...
        expect(result.match, `Float-only round-trip: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
    it('float-only gradient (HDR range) matches within 0.5%', () => {
        ...
        const decoded = decodeGainMapFromFloatEncoding(encoding);
        const result = compareImages(original, decoded, TOLERANCE_FLOAT_ONLY);
        expect(result.match, `Float-only gradient: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
    ...
    it('SDR quantized only (gain map float): round-trip within 1%', () => {
        ...
        expect(result.match, `SDR quantized round-trip: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
    it('full encode (quantized) → decode: rainbow within 0.5%', () => {
        ...
        expect(result.match, `Full round-trip: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
    it('full encode → decode: gradient (HDR range) within 0.5%', () => {
        ...
        const decoded = decodeGainMap(encoding);
        const result = compareImages(original, decoded, TOLERANCE_FULL);
        expect(result.match, `Full gradient: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
    it('full encode → decode: low DR (intensity 1) within 0.5%', () => {
        ...
        const decoded = decodeGainMap(encoding);
        const result = compareImages(original, decoded, TOLERANCE_FULL);
        expect(result.match, `Low-DR round-trip: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
    /**
    ...
    it('slow gradient 0.05–1.5: line-by-line validation (full quantized)', () => {
        ...
        const worstColsStr = worstCols
            ...
            .join('; ');
        expect(result.failures.length, `${slowGradientFailureMessage(result)} Worst columns by error: ${worstColsStr}`).toBe(0);
    });
});
//# sourceMappingURL=gainmapRoundtrip.test.js.map