...
import { compareImages, createGradientImage, encodeGainMap, encodeToJpeg, readJpegGainMap, writeJpegGainMap, } from '../index.js';
describe('readJpegGainMap', () => {
    it('round-trips: synthetic image → encodeGainMap → writeJpegGainMap → readJpegGainMap → compare within tolerance', () => {
        ...
        expect(result.match, `Round-trip: maxAbsoluteDelta=${result.maxAbsoluteDelta} mismatchedPixels=${result.mismatchedPixels}`).toBe(true);
    });
...
//# sourceMappingURL=readJpegGainMap.test.js.map