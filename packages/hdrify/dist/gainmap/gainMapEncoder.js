...
const defaultOffset = [1 / 64, 1 / 64, 1 / 64];
const defaultGamma = [1, 1, 1];
/**
 * Find max RGB value in HDR image (for backward compat / fallback).
 */
function findMaxHdrValue(data) {
    let max = 0;
 ...
 */
function findMaxContentBoostFromGains(data, totalPixels, toneMapping, linearRgbBuffer, offsetSdr, offsetHdr, exposure) {
 ...
 */
export function encodeGainMap(image, options = {}) {
    ensureNonNegativeFinite(image.data);
    const data = convertFloat32ToLinearColorSpace(image.data, image.width, image.height, image.linearColorSpace, 'linear-rec709');
    ...
    const toneMappingType = options.toneMapping ?? 'aces';
    ...
    if (maxContentBoost === undefined || maxContentBoost <= 0) {
        const fromGains = findMaxContentBoostFromGains(data, totalPixels, toneMapping, linearRgbBuffer, offsetSdr, offsetHdr, exposure);
    ...
    const gainMapMinLog = [minLog2, minLog2, minLog2];
    const gainMapMaxLog = [maxLog2, maxLog2, maxLog2];
    const hdrCapacityMin = Math.min(Math.max(0, gainMapMinLog[0]), Math.max(0, gainMapMinLog[1]), Math.max(0, gainMapMinLog[2]));
    const hdrCapacityMax = Math.max(Math.max(0, gainMapMaxLog[0]), Math.max(0, gainMapMaxLog[1]), Math.max(0, gainMapMaxLog[2]));
    const metadata = {
        gamma,
        offsetSdr,
        offsetHdr,
 ...
 */
export function encodeGainMapToFloat(image, options = {}) {
    ensureNonNegativeFinite(image.data);
    const data = convertFloat32ToLinearColorSpace(image.data, image.width, image.height, image.linearColorSpace, 'linear-rec709');
    const { width, height } = image;
    const totalPixels = width * height;
    ...
    const gamma = options.gamma ?? defaultGamma;
    const exposure = options.exposure ?? 1;
    const toneMappingTypeFloat = options.toneMapping ?? 'aces';
    ...
    if (maxContentBoostFloat === undefined || maxContentBoostFloat <= 0) {
        const fromGains = findMaxContentBoostFromGains(data, totalPixels, toneMappingFloat, linearRgbBuffer, offsetSdr, offsetHdr, exposure);
    ...
    // biome-ignore-end lint/style/noNonNullAssertion: indices bounds-checked by totalPixels * 4
    const gainMapMinLogFloat = [minLog2Float, minLog2Float, minLog2Float];
    const gainMapMaxLogFloat = [maxLog2Float, maxLog2Float, maxLog2Float];
    const hdrCapacityMinFloat = Math.min(Math.max(0, gainMapMinLogFloat[0]), Math.max(0, gainMapMinLogFloat[1]), Math.max(0, gainMapMinLogFloat[2]));
    const hdrCapacityMaxFloat = Math.max(Math.max(0, gainMapMaxLogFloat[0]), Math.max(0, gainMapMaxLogFloat[1]), Math.max(0, gainMapMaxLogFloat[2]));
    const metadata = {
        gamma,
        offsetSdr,
        offsetHdr,
...
//# sourceMappingURL=gainMapEncoder.js.map