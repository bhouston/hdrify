 ...
 */
export function readHdr(hdrBuffer, options = {}) {
 ...
 */
function RGBE_ReadHeader(buffer, offsetRef, opts) {
    ...
    function parseMetadataValue(val) {
    ...
    while (true) {
        ...
        if (resMatch) {
            ...
            throw new Error(`Unsupported resolution format: ${line.trim()}. Only -Y N +X M (standard orientation) is supported.`);
        }
    }
...
function decodeOldRLE(buffer, w, h) {
 ...
 */
export function hdrToLdr(hdrData, width, height, options = {}) {
 ...
 */
export function convertHDRToLDR(hdrBuffer, options = {}) {
    const hdrImage = readHdr(hdrBuffer);
    const fileExposure = hdrImage.metadata?.EXPOSURE;
...
//# sourceMappingURL=readHdr.js.map