...
describe('hdrWriter', () => {
    describe('writeHdr', () => {
        ...
        function buildGradient010() {
            ...
            return { width: steps, height: 1, linearColorSpace: 'linear-rec709', data };
        }
        function roundTripErrors(original, buffer) {
        ...
        it('RGBE round-trip: values 0 to 10 in 0.01 steps (R channel only) within 8%', () => {
            ...
            expect(failures, `RGBE round-trip should be within 8% for 0..10. Failures: ${failures
                .slice(0, 20)
                .map((f) => `value=${f.value.toFixed(3)} relErr=${(f.relErr * 100).toFixed(2)}%`)
                .join('; ')}${failures.length > 20 ? ` ... and ${failures.length - 20} more` : ''}`).toHaveLength(0);
        });
    });
});
//# sourceMappingURL=writeHdr.test.js.map