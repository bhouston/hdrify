        ...
        describe(`${fromSpace} → ${toSpace} → ${fromSpace}`, () => {
            for (const channel of GRADIENT_CHANNELS) {
                it(`${channel}-to-black gradient round-trips within 1%`, () => {
                    ...
                    const inFromSpace = original.linearColorSpace === fromSpace ? original : convertLinearColorSpace(original, fromSpace);
...
describe('linear → display → linear round-trip', () => {
    for (const linearSpace of ['linear-rec709', 'linear-p3', 'linear-rec2020']) {
        const displaySpace = LINEAR_TO_DISPLAY[linearSpace];
        describe(`${linearSpace} → ${displaySpace} → ${linearSpace}`, () => {
            for (const channel of GRADIENT_CHANNELS) {
                it(`${channel}-to-black gradient round-trips within 1%`, () => {
                    const original = makeGradient(channel);
                    const inLinearSpace = original.linearColorSpace === linearSpace ? original : convertLinearColorSpace(original, linearSpace);
                    const displayImage = convertLinearToDisplay(inLinearSpace, displaySpace);
                    const back = convertDisplayToLinear(displayImage.data, displayImage.width, displayImage.height, displaySpace, linearSpace);
...
function roundtripMessage(result, channel, from, to) {
    if (result.match)
        return '';
    const parts = [
...
//# sourceMappingURL=convertRoundtrip.test.js.map