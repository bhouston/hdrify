...
describe('createCieColorWedgeImage', () => {
    ...
    it('channel r: only R has wedge data, G and B are 0', () => {
        ...
        let maxR = 0;
        for (let i = 0; i < img.data.length; i += 4)
            maxR = Math.max(maxR, img.data[i] ?? 0);
    ...
    it('channel g: only G has wedge data, R and B are 0', () => {
        ...
        for (let i = 1; i < img.data.length; i += 4)
            maxG = Math.max(maxG, img.data[i] ?? 0);
    ...
    it('channel b: only B has wedge data, R and G are 0', () => {
        ...
        let maxB = 0;
        for (let i = 2; i < img.data.length; i += 4)
            maxB = Math.max(maxB, img.data[i] ?? 0);
        expect(maxB).toBeGreaterThan(0);
    });
});
//# sourceMappingURL=createCieColorWedgeImage.test.js.map