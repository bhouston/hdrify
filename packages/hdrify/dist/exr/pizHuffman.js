 ...
 */
import { HUF_DECBITS, HUF_DECMASK, HUF_DECSIZE, HUF_ENCSIZE, INT8_SIZE, INT32_SIZE, LONG_ZEROCODE_RUN, SHORT_ZEROCODE_RUN, SHORTEST_LONG_RUN, } from './exrConstants.js';
...
function countFrequencies(freq, data, n) {
    for (let i = 0; i < HUF_ENCSIZE; i++)
        freq[i] = 0;
    for (let i = 0; i < n; i++) {
        const d = data[i];
...
function hufBuildEncTable(frq, im, iM) {
    const hlink = new Array(HUF_ENCSIZE);
    const fHeap = [];
    const scode = new Array(HUF_ENCSIZE).fill(0);
    im.value = 0;
    while (!(frq[im.value] ?? 0))
        im.value++;
    let nf = 0;
    ...
    fHeap.push(iM.value);
    nf++;
    const FHeapCompare = (a, b) => (frq[a] ?? 0) > (frq[b] ?? 0) || ((frq[a] ?? 0) === (frq[b] ?? 0) && a > b);
    fHeap.sort((a, b) => (FHeapCompare(a, b) ? 1 : -1));
    while (nf > 1) {
        const mm = fHeap.shift();
        const m = fHeap[0];
        if (mm === undefined || m === undefined)
            break;
        fHeap.splice(0, 1);
        ...
        for (let j = m;;) {
            ...
            scode[j] = s;
            if (s > 58)
                throw new Error('Huffman code length > 58');
            const next = hlink[j];
            if (next === j) {
                hlink[j] = mm;
                break;
...
function outputBits(nBits, bits, c, lc, out) {
    c.value <<= nBits;
    lc.value += nBits;
    c.value |= bits;
    while (lc.value >= 8) {
...
function hufPackEncTable(hcode, im, iM, out) {
    const c = { value: 0 };
    const lc = { value: 0 };
    let currentIm = im;
    while (currentIm <= iM) {
        ...
        if (l === 0) {
            ...
            while (currentIm < iM && zerun < LONGEST_LONG_RUN) {
                ...
                if (hufLength(hcode[currentIm + 1]) > 0)
                    break;
                currentIm++;
                zerun++;
            }
    ...
    if (lc.value > 0)
        out.push((c.value << (8 - lc.value)) & 0xff);
}
function outputCode(code, c, lc, out) {
    outputBits(hufLength(code), hufCode(code), c, lc, out);
}
function sendCode(sCode, runCount, runCode, c, lc, out) {
...
function hufEncode(hcode, inData, ni, rlc, out) {
    const c = { value: 0 };
    const lc = { value: 0 };
 ...
 */
export function hufCompress(raw) {
    const n = raw.length;
    if (n === 0)
 ...
 */
export function hufUncompress(uInt8Array, inDataView, inOffset, nCompressed, outBuffer, nRaw) {
    ...
    if (im < 0 || im >= HUF_ENCSIZE || iM < 0 || iM >= HUF_ENCSIZE) {
        throw new Error(`Something wrong with HUF_ENCSIZE: im=${im}, iM=${iM}, HUF_ENCSIZE=${HUF_ENCSIZE}, compressedSize=${nCompressed}, offset=${inOffset.value}`);
    }
    const freq = new Array(HUF_ENCSIZE);
...
//# sourceMappingURL=pizHuffman.js.map