...
import { FLOAT32_SIZE, INT16_SIZE, INT32_SIZE, NO_COMPRESSION, PIZ_COMPRESSION, PXR24_COMPRESSION, RLE_COMPRESSION, ULONG_SIZE, ZIP_COMPRESSION, ZIPS_COMPRESSION, } from './exrConstants.js';
import { parseExrHeader } from './exrHeader.js';
import { decodeFloat16 } from './halfFloat.js';
function getPixelTypeSize(pixelType) {
    const UINT = 0;
    const HALF = 1;
    const FLOAT = 2;
    switch (pixelType) {
...
function readChannelValue(dataView, offset, pixelType) {
    const UINT = 0;
    const HALF = 1;
    const FLOAT = 2;
    switch (pixelType) {
...
function getChannelSemanticName(name) {
 ...
 */
export function readExr(exrBuffer) {
    ...
    if (!hasRgb && !hasLuma) {
        throw new Error('Unsupported EXR channel layout. This reader requires R/G/B channels or a luminance channel (Y/L/luminance).');
    }
    ...
    const blockHeight = compression === PIZ_COMPRESSION
        ? 32
    ...
    for (let blockIdx = 0; blockIdx < blockCount; blockIdx++) {
        ...
        if (scanlineBlockOffset >= exrBuffer.length || scanlineBlockOffset < 0) {
            throw new Error(`Invalid scanline block offset ${scanlineBlockOffset} for block ${blockIdx} (file size: ${exrBuffer.length})`);
        }
        ...
        if (dataSize <= 0 || dataSize > available) {
            ...
            if (looksLikeFormatMismatch) {
                throw new Error(`Unsupported or invalid EXR format: scanline block ${blockIdx} has invalid data size (${dataSize} bytes, ${available} available). ` +
                    `This file may use a compression or layout not supported by this reader. Supported: none, RLE, ZIPS, ZIP, PIZ, PXR24.`);
            }
            throw new Error(`Invalid scanline block data size: ${dataSize} at offset ${scanlinePos - 4} (file size: ${exrBuffer.length}, available: ${available})`);
        }
        ...
        if (compression === NO_COMPRESSION) {
            ...
            if (dataSize <= 0 || scanlinePos + dataSize > exrBuffer.length) {
                throw new Error(`Invalid PXR24 data size: ${dataSize} at offset ${scanlinePos} (file size: ${exrBuffer.length})`);
            }
            const compressedData = new Uint8Array(exrBuffer.buffer, exrBuffer.byteOffset + scanlinePos, dataSize);
        ...
        const blockDataView = new DataView(decompressedData.buffer, decompressedData.byteOffset, decompressedData.byteLength);
        ...
        const isPlanar = compression === RLE_COMPRESSION ||
        ...
        for (let lineInBlock = 0; lineInBlock < linesInBlock; lineInBlock++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                if (usePxr24RgbBlockOrder) {
                    channelValues.r = readChannelValue(blockDataView, lineOffset + 2 * width * bytesPerChannel + x * bytesPerChannel, referencePixelType);
                    channelValues.g = readChannelValue(blockDataView, lineOffset + 1 * width * bytesPerChannel + x * bytesPerChannel, referencePixelType);
                    channelValues.b = readChannelValue(blockDataView, lineOffset + 0 * width * bytesPerChannel + x * bytesPerChannel, referencePixelType);
                }
                else {
    ...
    const linearColorSpace = chromaticities &&
...
//# sourceMappingURL=readExr.js.map