 ...
 */
export function bitmapFromData(data, nData, bitmap, minNonZero, maxNonZero) {
 ...
 */
export function forwardLutFromBitmap(bitmap, lut) {
    let k = 0;
 ...
 */
export function applyLutForward(lut, data, nData) {
    for (let i = 0; i < nData; i++) {
        const v = data[i];
        if (v !== undefined) {
            const lv = lut[v];
 ...
 */
function wav2Encode(buffer, j, nx, ox, ny, oy, mx) {
    const w14 = mx < 1 << 14;
    const n = nx > ny ? ny : nx;
    let p = 1;
    let p2 = 2;
 ...
 */
function rearrangeToChannelPlanar(src, width, blockHeight, channels) {
 ...
 */
export function compressPizBlock(rawHalfFloatInterleaved, width, blockHeight, channels) {
    const numChannels = channels.length;
    const pixelsPerChannel = width * blockHeight;
...
//# sourceMappingURL=compressPiz.js.map