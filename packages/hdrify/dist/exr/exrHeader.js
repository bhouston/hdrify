 ...
 */
import { COMPRESSION_NAMES, EXR_MAGIC, FLOAT32_SIZE, INT8_SIZE, INT16_SIZE, INT32_SIZE, NO_COMPRESSION, SUPPORTED_COMPRESSION, } from './exrConstants.js';
 ...
 */
export function parseExrHeader(exrBuffer) {
    ...
    if (isMultiPart || isTiled || isDeepData) {
        throw new Error('Multi-part, tiled, and deep data EXR files are not supported. This reader supports single-part scanline images only.');
    }
    ...
    const displayWindow = header.displayWindow;
    const dataWindow = header.dataWindow;
    const channels = header.channels;
    const compression = header.compression ?? NO_COMPRESSION;
    ...
    if (!displayWindow || !dataWindow || !channels || channels.length === 0) {
        ...
        throw new Error(`Invalid EXR file: missing required header attributes. displayWindow: ${hasDisplayWindow}, dataWindow: ${hasDataWindow}, channels: ${hasChannels ? channels.length : 0}, header keys: ${Object.keys(header).join(', ')}`);
    }
    return {
        header: {
            header,
...
//# sourceMappingURL=exrHeader.js.map