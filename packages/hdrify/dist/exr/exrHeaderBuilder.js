...
function addAttribute(parts, name, type, value) {
 ...
 */
export function buildExrHeader(options = {}) {
    const { width = 10, height = 10, compression = NO_COMPRESSION, channels = [{ name: 'R', pixelType: FLOAT }, { name: 'G' }, { name: 'B' }, { name: 'A' }], extraAttributes = [], omitAttributes = [], } = options;
    const parts = [];
    const addIfNotOmitted = (name, fn) => {
        if (!omitAttributes.includes(name))
            fn();
    };
    // displayWindow (box2i)
 ...
 */
export function buildExrHeaderForParsing(options = {}) {
...
//# sourceMappingURL=exrHeaderBuilder.js.map