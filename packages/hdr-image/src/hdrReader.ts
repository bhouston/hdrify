 ...
 *
 * @param hdrBuffer - Buffer containing HDR file data
 ...
 */
export function parseHDRFile(hdrBuffer: Buffer, options: ParseHDROptions = {}): FloatImageData {
  const { headerStrict = true, output = 'raw' } = options;

  // Convert Buffer to Uint8Array for processing
  const byteArray = new Uint8Array(hdrBuffer.buffer, hdrBuffer.byteOffset, hdrBuffer.byteLength);
...
function decodeOldRLE(buffer: Uint8Array, w: number, h: number): Uint8Array {
  ...
  while (outIdx < output.length && pos < buffer.length) {
    ...
    const g = buffer[pos++] ?? 0;
    const b = buffer[pos++] ?? 0;
    const e = buffer[pos++] ?? 0;
    ...
    if (r === 255 && g === 255 && b === 255) {
      ...
      while (pos + 4 <= buffer.length) {
        const r2 = buffer[pos] ?? 0;
        const g2 = buffer[pos + 1] ?? 0;
        const b2 = buffer[pos + 2] ?? 0;
        const e2 = buffer[pos + 3] ?? 0;
      ...
      for (let i = 0; i < count && outIdx < output.length; i++) {
        output[outIdx++] = prev[0] ?? 0;
        output[outIdx++] = prev[1] ?? 0;
        output[outIdx++] = prev[2] ?? 0;
        output[outIdx++] = prev[3] ?? 0;
      }
    } else {
      const rn = ((r ?? 0) & 0xff) as number;
      const gn = ((g ?? 0) & 0xff) as number;
      const bn = ((b ?? 0) & 0xff) as number;
      const en = ((e ?? 0) & 0xff) as number;
      prev[0] = rn;
      prev[1] = gn;
      prev[2] = bn;
      prev[3] = en;
...
function RGBE_ReadPixels_RLE(buffer: Uint8Array, w: number, h: number): Uint8Array {
  ...
  if (
    ...
    if (pixelBuffer.length !== expected) {
      throw new Error(
        `HDR Bad File Format: expected ${expected} bytes (${w}x${h}), got ${pixelBuffer.length}`,
      );
    }
    return new Uint8Array(pixelBuffer);
  }

  const bufferWidth = ((pixelBuffer[2] ?? 0) << 8) | (pixelBuffer[3] ?? 0);
  ...
  while (num_scanlines > 0 && pos < pixelBuffer.byteLength) {
    ...
    rgbeStart[0] = pixelBuffer[pos++] ?? 0;
    rgbeStart[1] = pixelBuffer[pos++] ?? 0;
    rgbeStart[2] = pixelBuffer[pos++] ?? 0;
    rgbeStart[3] = pixelBuffer[pos++] ?? 0;
    ...
    for (let i = 0; i < l; i++) {
      let off = 0;
      data_rgba[outputOffset] = scanline_buffer[i + off] ?? 0;
      off += scanline_width;
      data_rgba[outputOffset + 1] = scanline_buffer[i + off] ?? 0;
      off += scanline_width;
      data_rgba[outputOffset + 2] = scanline_buffer[i + off] ?? 0;
      off += scanline_width;
      data_rgba[outputOffset + 3] = scanline_buffer[i + off] ?? 0;
      outputOffset += 4;
    }
 ...
 * @returns Buffer containing uint8 RGB data ready for image encoding
 */
export function hdrToLdr(hdrData: Float32Array, width: number, height: number, options: HDRToLDROptions = {}): Buffer {
  const exposure = options.exposure ?? 1.0;
  const gamma = options.gamma ?? 2.2;
  const totalPixels = width * height;
  const ldrData = Buffer.allocUnsafe(totalPixels * 3);
...
): { width: number; height: number; ldrData: Buffer } {
