...
The workflow is installed in `.github/workflows/release.yml`. npm trusted publishing is already configured for all three packages, so any manual dispatch on `main` publishes.
...
4. Dispatch `Release` on `main` (see above) when ready to publish.
