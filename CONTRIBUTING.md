...
2. Fetch origin and branch from `origin/dev`. Use `feature/<issue>-<short-description>` for features; `fix/`, `chore/`, `docs/`, `refactor/`, and `test/` are also accepted. Example: `feature/42-batch-export`.
3. Implement and validate the acceptance criteria. Every commit must use Conventional Commits. Reference the issue in the commit body where useful. Never commit directly to `main` or `dev`.
...
5. Push the branch and open a PR against **dev**. Give the PR a Conventional Commit title and include `Closes #<issue>`, a description of the resulting behavior, and validation results. Do not merge your own work unless the maintainer requested a merge.
6. Release only through a PR from this repository’s **dev** branch to **main**, titled `chore(release): merge dev into main`. Use a **merge commit**, never squash or rebase the release PR: semantic-release needs the individual feature commits. After release, merge `origin/main` back into `dev` to keep history aligned.

GitHub automatically closes referenced issues when their closing commits reach the default branch (`main`). A PR to `dev` carries the closing reference but does not close the issue immediately. Preserve `Closes #<issue>` in squash commit bodies, or include the delivered issue list in the release PR body.
